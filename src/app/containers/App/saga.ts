...
export function* callAPI(url: string, params: AxiosRequestConfig) {
  try {
    ...
    if (!response) {
      ...
      switch (status) {
        case 401:
          yield put(notify.showError('Ошибка авторизации, попробуйте авторизоваться снова'));
          return { status };
        case 403:
          yield put(notify.showError('У вас нет прав для просмотра этой страницы'));
          return { status };
        default:
          yield put(notify.showError('Внутрення ошибка сервера, попробуйте позже'));
          return { status };
      }
    }
  }
